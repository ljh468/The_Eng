...
public class MongoQuizMapper implements IMongoQuizMapper{
	...
	@Override
	public QuizInfoDTO getQuizInfo(String user_id, String newsUrl) {
		
		DBObject query = new BasicDBObject("user_id", user_id).append("newsUrl", newsUrl);
		
		DBCursor cursor = mongodb.getCollection("userQuizInfo").find(query);
		
		QuizInfoDTO qDTO = null;
		if(cursor.size()==0) {
			qDTO = new QuizInfoDTO(user_id, 0, newsUrl);
	...
	@Override
	public void nextQuiz(String user_id, String newsUrl) {
		
		DBObject query = new BasicDBObject("user_id", user_id).append("newsUrl", newsUrl);
		DBObject update = new BasicDBObject("idx", new BasicDBObject("$inc", 1));  // #inc는 숫자를 하나씩 올려서 업데이트
